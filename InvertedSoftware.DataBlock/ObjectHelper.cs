...
namespace InvertedSoftware.DataBlock.Core
{
    ...
    public class ObjectHelper
    {
        ...
        internal struct DataObjectInfo
        {
            ...
            public FrozenDictionary<string, MapToColumn> MappingAttributes { get; set; }
            public FrozenDictionary<string, CrudField> AllAttributes { get; set; }
            public FrozenDictionary<string, ForeignKeyAttribute> ForeignKeyAttributes { get; set; }
            public FrozenDictionary<string, DatabaseGeneratedAttribute> IdentityAttributes { get; set; }
            public FrozenDictionary<string, Action<object, object>> SetMethods { get; set; }
            public FrozenDictionary<string, Func<object, object>> GetMethods { get; set; }
        }

        /// <summary>
        /// local cache for metadata.
        /// </summary>
        private static FrozenDictionary<string, DataObjectInfo> ObjectInfoCache = new ConcurrentDictionary<string, DataObjectInfo>().ToFrozenDictionary<string, DataObjectInfo>();

        /// <summary>
        /// Local cache for field names in queries.
        /// </summary>
        private static FrozenDictionary<string, List<string>> QueryColumnNamesCache = new ConcurrentDictionary<string, List<string>>().ToFrozenDictionary<string, List<string>>();
        ...
        internal static DataObjectInfo GetDataObjectInfo<T>()
        {
            ...
            if (!ObjectInfoCache.TryGetValue(name, out dataObjectInfo))
            {
                Type type = typeof(T);
                // Fill in properties
                dataObjectInfo = new DataObjectInfo()
                {
                    ...
                    MappingAttributes = new Dictionary<string, MapToColumn>().ToFrozenDictionary<string, MapToColumn>(),
                    AllAttributes = new Dictionary<string, CrudField>().ToFrozenDictionary<string, CrudField>(),
                    ForeignKeyAttributes = new Dictionary<string, ForeignKeyAttribute>().ToFrozenDictionary<string, ForeignKeyAttribute>(),
                    IdentityAttributes = new Dictionary<string, DatabaseGeneratedAttribute>().ToFrozenDictionary<string, DatabaseGeneratedAttribute>(),
                    SetMethods = new Dictionary<string, Action<object, object>>().ToFrozenDictionary<string, Action<object, object>>(),
                    GetMethods = new Dictionary<string, Func<object, object>>().ToFrozenDictionary<string, Func<object, object>>()
                };
                ...
                foreach (var property in dataObjectInfo.Properties)
                {
                    ...
                    if (mapAtt != null)
                    {
                        var tmpMappingAttributes = dataObjectInfo.MappingAttributes.ToDictionary<string, MapToColumn>();
                        if(tmpMappingAttributes.TryAdd(property.Name, mapAtt))
                        dataObjectInfo.MappingAttributes = tmpMappingAttributes.ToFrozenDictionary<string, MapToColumn>();
                    }
                    ...
                    if (dataAtt != null)
                    {
                        var tmpAllAttributes = dataObjectInfo.AllAttributes.ToDictionary<string, CrudField>();
                        if(tmpAllAttributes.TryAdd(property.Name, dataAtt))
                        dataObjectInfo.AllAttributes = tmpAllAttributes.ToFrozenDictionary<string, CrudField>();
                    }
                    ...
                    if (fkAttribute != null)
                    {
                        var tmpForeignKeyAttributes = dataObjectInfo.ForeignKeyAttributes.ToDictionary<string, ForeignKeyAttribute>();
                        if(tmpForeignKeyAttributes.TryAdd(fkAttribute.Name, fkAttribute))
                        dataObjectInfo.ForeignKeyAttributes = tmpForeignKeyAttributes.ToFrozenDictionary<string, ForeignKeyAttribute>();
                    }
                    ...
                    {
                        var tmpIdentityAttributes = dataObjectInfo.IdentityAttributes.ToDictionary<string, DatabaseGeneratedAttribute>();
                        if (tmpIdentityAttributes.TryAdd(property.Name, IDAttribute))
                            dataObjectInfo.IdentityAttributes = tmpIdentityAttributes.ToFrozenDictionary<string, DatabaseGeneratedAttribute>();
                    }
                    ...
                    if (setMethod != null)
                    {
                        ...
                        if (mapAtt == null)
                        {
                            var tmpSetMethods = dataObjectInfo.SetMethods.ToDictionary<string, Action<object, object>>();
                            if(tmpSetMethods.TryAdd(property.Name, (Action<object, object>)setter.CreateDelegate(typeof(Action<object, object>))))
                                dataObjectInfo.SetMethods = tmpSetMethods.ToFrozenDictionary<string, Action<object, object>>();
                        }
                        else
                        {
                            var tmpSetMethods = dataObjectInfo.SetMethods.ToDictionary<string, Action<object, object>>();
                            if (tmpSetMethods.TryAdd(mapAtt.ColumnName, (Action<object, object>)setter.CreateDelegate(typeof(Action<object, object>))))
                                dataObjectInfo.SetMethods = tmpSetMethods.ToFrozenDictionary<string, Action<object, object>>();
                        }
                    }
                    ...
                    if (getMethod != null)
                    {
                        ...
                        if (mapAtt == null)
                        {
                            var tmpGetMethods = dataObjectInfo.GetMethods.ToDictionary<string, Func<object, object>>();
                            if (tmpGetMethods.TryAdd(property.Name, (Func<object, object>)getter.CreateDelegate(typeof(Func<object, object>))))
                                dataObjectInfo.GetMethods = tmpGetMethods.ToFrozenDictionary<string, Func<object, object>>();
                        }
                        else
                        {
                            var tmpGetMethods = dataObjectInfo.GetMethods.ToDictionary<string, Func<object, object>>();
                            if (tmpGetMethods.TryAdd(mapAtt.ColumnName, (Func<object, object>)getter.CreateDelegate(typeof(Func<object, object>))))
                                dataObjectInfo.GetMethods = tmpGetMethods.ToFrozenDictionary<string, Func<object, object>>();
                        }   
                    }
                }

                var tmpObjectInfoCache = ObjectInfoCache.ToDictionary();
                tmpObjectInfoCache.TryAdd(name, dataObjectInfo);
                ObjectInfoCache = tmpObjectInfoCache.ToFrozenDictionary();
            }
        ...
        internal static List<string> GetColumnNames(SqlDataReader reader, string sprocName, bool useCache = true)
        {
            if (useCache)
            {
                ...
                if (!QueryColumnNamesCache.TryGetValue(sprocName, out columnNames)){
                    columnNames = GetColumnNames(reader, sprocName);

                    var tmpQueryColumnNamesCache = QueryColumnNamesCache.ToDictionary<string, List<string>>();
                    if(tmpQueryColumnNamesCache.TryAdd(sprocName, columnNames))
                        QueryColumnNamesCache = tmpQueryColumnNamesCache.ToFrozenDictionary<string, List<string>>();
                }
                return columnNames;
            }
            else
        ...
        private static List<T2> GetForeignKeyFilteredList<T1, T2>(T1 parent, List<T2> children, string foreignKey)
        {
            ...
            InvertedSoftware.DataBlock.Core.ObjectHelper.DataObjectInfo childDataObjectInfo = ObjectHelper.GetDataObjectInfo<T2>();
